...
def check_sentLength(sentToFinal):
    ...
    for i in range(max_len + 1):
        ...
        print('%2d: %10d %.3f%% %.3f%%' % (
        i, sent_lengths.get(i, 0), sent_lengths.get(i, 0) * 100.0 / sum_len, acc * 100.0 / sum_len))
...
def encode_captions(sentences, wtoi, params):
    ...
    if max_length == None:
        if params['dataset'] in ['refcoco', 'refclef', 'refcoco+']:
            max_length = 10
        elif params['dataset'] in ['refcocog', 'refgta', "cityscapes"]:
            max_length = 20
        elif params["dataset"] in ["talk2car"]:
            max_length = 15
        else:
            print("No max length for {}".format(params["dataset"]))
            exit()
    ...
    for image_id in refer.Imgs:
        ...
        for ann_id in ann_ids:
            ...
            anns += [{'ann_id': ann_id,
                      ...
                      "location": ann["attributes"]["location"],
                      "action": ann["attributes"]["action"],
                      "count": ann["attributes"]["count"]}]
            h5_id += 1
