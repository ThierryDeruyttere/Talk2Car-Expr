...
class DataLoader:
    def __init__(self, opt):
        ...
        self.graphs = None
        self.NUM_ATTRS = 26 + 38 # (color, location, action) + (count)
    ...
    def fetch_feats(self, batch_ann_ids, expand_size, opt):
        ...
        sp_ann_feats = np.tile(
            self.feats['sp_ann'][h5_id_list].reshape((batch_size, 1, 6, 6, -1)), (1, expand_size, 1, 1)
        ).reshape((batch_size * expand_size, 6, 6, -1))
        ...
        attrs = self.fetch_attrs(batch_ann_ids, expand_size, opt)
        attrs_feats = np.tile(attrs.reshape((batch_size, 1, -1)), (1, expand_size, 1)).reshape((batch_size*expand_size, -1))
        feats = np.concatenate([cxt_feats, ann_feats, l_feats, df_feats, dlf_feats, attrs_feats], axis=1)
        classes = np.tile(np.array(classes).reshape((batch_size, 1, -1)), (1, expand_size, 1)).reshape((batch_size*expand_size, -1))
        return feats, classes, sp_ann_feats

    def fetch_attrs(self, batch_ann_id, expand_size, opt):
        batch_size = len(batch_ann_id)
        attrs = np.zeros((batch_size, self.NUM_ATTRS), dtype=np.float32)
        ...
        for i, ann_id in enumerate(batch_ann_id):
            ...
            color = self.Anns[ann_id]["color"]["pred"]
            location = self.Anns[ann_id]["location"]["pred"]
            action = self.Anns[ann_id]["action"]["pred"]
            count = self.Anns[ann_id]["count"]
            count_feats = np.zeros(38, dtype=np.float32)
            count_feats[count-1] = 1
            attr_lst = []
            attr_lst += color
            attrs[i, :] = np.hstack([color, location, action, count_feats])
        return attrs
    ...
    def sample_neg_ids(self, pos_ann_ids, opt):
        ...
        for pos_ann_id in pos_ann_ids:
            ...
            for k in range(opt['seq_per_ref']):
                ...
                neg_sent_cat.append(self.Refs[neg_ref_id]["category_id"])
        return neg_ann_ids, neg_sent_ids, neg_sent_cat
    ...
    def getBatch(self, split, opt):
        ...
        feats, pos_classes, feat_maps = self.fetch_feats(batch_ann_ids, seq_per_ref, opt)
        seqz = self.fetch_seqs(batch_sent_ids, {'pad_zero':'end'})
        zseq = self.fetch_seqs(batch_sent_ids, {'pad_zero':'front'})
        neg_ann_ids, neg_sent_ids, neg_feats, neg_seqz, neg_zseq, neg_sent_cat = None, None, None, None, None, None
        if sample_neg>0:
            neg_ann_ids, neg_sent_ids, neg_sent_cat = self.sample_neg_ids(batch_ann_ids, opt)
            neg_feats, neg_classes, neg_feat_maps = self.fetch_feats(neg_ann_ids, 1, opt)
        ...
        data['feats'] = feats
        data['feat_maps'] = feat_maps
        data['classes'] = pos_classes
        data['seqz'] = seqz
        data['zseq'] = zseq
        data['neg_ann_ids'] = neg_ann_ids
        data['neg_sent_ids'] = neg_sent_ids
        data['neg_feats'] = neg_feats
        data['neg_feat_maps'] = neg_feat_maps
        data['neg_seqz'] = neg_seqz
        data['neg_zseq'] = neg_zseq
        data['neg_obj_classes'] = neg_classes
        data['neg_lang_classes'] = neg_sent_cat
        if self.dataset == 'refgta':
    ...
    def getImageBatch(self, split, opt):
        ...
        ann_ids = image['ann_ids']
        feats, classes, feat_maps = self.fetch_feats(ann_ids, 1, opt)
        sent_ids = []
        gd_ixs = []
        rank = []
    ...
    def getTestBatch(self, split, opt):
        ...
        feats, classes, feat_maps = self.fetch_feats(batch_ann_ids, 1, opt)
        seqz = self.fetch_seqs(batch_sent_ids, {'pad_zero':'end'})
        zseq = self.fetch_seqs(batch_sent_ids, {'pad_zero':'front'})
        data = {}
