...
def main():
    ...
    if args.TEST:
        ...
        model = AttrExprGen.load_from_checkpoint(files[0], vocab=vocab, attr_vocab=attr_vocab,
                                                 class_vocab=class_vocab, hparams=hp)
        logger = None
        checkpoint_callback = None
    else:
        model = AttrExprGen(vocab, attr_vocab=attr_vocab, class_vocab=class_vocab, hparams=args)
        # setup the logger
        name = ["NEW_DIFF",'{}'.format(args.dataset)]
        if args.DEBUG:
        ...
        if args.use_reinforcer != "":
            name.append("UseReinforcer_{}".format(args.use_reinforcer.replace("/","@")))
        if args.mmi_loss_version != "v1":
            name.append("mmi_loss_v2")
        if args.mmi_loss_margin:
            name.append("mmi_loss_margin{}".format(args.mmi_loss_margin))
        if args.vlambda_weight:
            name.append("vlambda_weight{}".format(args.vlambda_weight))
        if args.llambda_weight:
            name.append("llambda_weight{}".format(args.llambda_weight))
...
if __name__ == '__main__':
    ...
    parser = AttrExprGen.add_model_specific_args(parser)
