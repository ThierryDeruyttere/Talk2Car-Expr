...
from misc.eval_utils import compute_margin_loss, computeLosses, calc_rank_loss, calc_rank_acc, compute_margin_loss_dist
...
def train_all(params):
    ...
    lm = LanguageModel(len(loader.ix_to_word), loader.seq_length, attr_size=NUM_ATTRS)
    ...
    min_val_loss = 100
    no_improvements = 0
    stop_after_x_epochs_w_no_improvement = 10
    while True:
        ...
        logprobs = lm(vis_feats, seqz, lang_last_ind, attrs=attrs)


        # emb loss
        pairSim, vis_unpairSim, lang_unpairSim = F.split_axis(cossim, 3, axis=0)
        pairCats = xp.array(data["classes"]).reshape(-1)
        vis_unpairCats = xp.array(data["neg_obj_classes"]).reshape(-1)
        #langCats = xp.array(data["classes"])
        lang_unpairCats = xp.array(data["neg_lang_classes"])
        emb_flows = {'vis':[pairSim, vis_unpairSim], "vis_cats": [pairCats, vis_unpairCats],
                     'lang':[pairSim, lang_unpairSim], "lang_cats": [pairCats, lang_unpairCats],
                     'c_oi': pairCats,
                     'c_oj': vis_unpairCats,
                     'c_ri': pairCats,
                     'c_rk': lang_unpairCats,
                     'd_ri_oi': pairSim,
                     'd_ri_oj': vis_unpairSim,
                     'd_rk_oi': lang_unpairSim
                     }
        emb_loss  = emb_crits(emb_flows, params['lm_margin_same_cat'], params['lm_margin_wrong_cat'])
        ...
        lm_flows = {'T':pairP, 'visF':[pairP, vis_unpairP],#, 'langF':[pairP, lang_unpairP]}
                    "visF_cats": [pairCats, vis_unpairCats],
                    'c_oi': pairCats,
                    'c_ok': vis_unpairCats,
                    'P(ri|ok)': vis_unpairP,
                    'P(ri|oi)': pairP
        ...
        lm_loss = lm_crits(lm_flows, num_labels, params['lm_margin_same_cat'], params['lm_margin_wrong_cat'],
                             vlamda=params['vis_rank_weight'])
        
        loss = emb_loss + lm_loss
        #print(emb_loss, lm_loss)
        ...
        if iteration % params['losses_log_every']==0:
            ...
            print('{} iter : train loss {}'.format(iteration,loss.data))
        ...
        if (iteration % params['save_checkpoint_every'] == 0 and iteration >0):
            ...
            while True:
                ...
                for i, sent_id in enumerate(sent_ids):
                    ...
                    cossim, vis_emb_feats = cca(vis_enc_feats, lang_enc_feats)
                    vis_feats = vis_combine(vis_enc_feats, vis_emb_feats)
                    logprobs = lm(vis_feats, sent_seqz, one_last_ind, attrs=attrs).data
                    
                    gd_ix = gd_ixs[i]
                    #lm_generation_loss = lm_crits({'T':logprobs[:, gd_ix, xp.newaxis]},
                    #                              {'T':one_last_ind[gd_ix,np.newaxis]}, params['lm_margin'],
                    #         vlamda=0, llamda=0).data

                    lm_generation_loss = lm_crits({'T':logprobs[:, gd_ix, xp.newaxis]},
                                                  {'T':one_last_ind[gd_ix,np.newaxis]},
                                                  params['lm_margin_same_cat'],
                                       params['lm_margin_wrong_cat'],
                                       vlamda=0).data
                    ...
                    emb_margin_loss, pos_sc, max_neg_sc = compute_margin_loss_dist(cossim.data, gd_ix, params['emb_margin'])
            ...
            if min_val_loss>fin_val_loss:
                ...
                no_improvements += 1
                if no_improvements >= stop_after_x_epochs_w_no_improvement:
                    exit()
