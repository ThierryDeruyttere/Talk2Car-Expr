...
class AttrLSTM(chainer.Chain):
    ...
    def __init__(self, vis_size, word_size, rnn_size, dropout_ratio, attr_size):
        super(AttrLSTM, self).__init__(
            ...
            ia=L.Linear(attr_size, rnn_size, nobias=True),
            iv=L.Linear(vis_size, rnn_size, nobias=True),

            fx=L.Linear(word_size, rnn_size, nobias=True),
            fh=L.Linear(rnn_size, rnn_size, nobias=True),
            fa=L.Linear(attr_size, rnn_size, nobias=True),
            fv=L.Linear(vis_size, rnn_size, nobias=True),

            ox=L.Linear(word_size, rnn_size, nobias=True),
            oh=L.Linear(rnn_size, rnn_size, nobias=True),
            oa=L.Linear(attr_size, rnn_size, nobias=True),
            ov=L.Linear(vis_size, rnn_size, nobias=True),

            cx=L.Linear(word_size, rnn_size, nobias=True),
            ch=L.Linear(rnn_size, rnn_size, nobias=True),
            ca=L.Linear(attr_size, rnn_size, nobias=True),
            cv=L.Linear(vis_size, rnn_size, nobias=True),
        )
    ...
    def __call__(self, vis=None, sos=None, word=None, attrs=None):
        ...
        i_t = F.sigmoid(self.ix(x_t) + self.ih(self.h) + self.ia(attrs) + self.iv(vis))
        f_t = F.sigmoid(self.fx(x_t) + self.fh(self.h) + self.fa(attrs) + self.fv(vis))
        o_t = F.sigmoid(self.ox(x_t) + self.oh(self.h) + self.oa(attrs) + self.ov(vis))

        c_tilde_t = F.tanh(self.cx(x_t) + self.ch(self.h) + self.ca(attrs) + self.cv(vis))
