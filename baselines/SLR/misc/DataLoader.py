...
class DataLoader:
    ...
    def getBatch(self, split, opt):
        ...
        feats, pos_classes = self.fetch_feats(batch_ann_ids, seq_per_ref, opt)
        ...
        if sample_neg>0:
            ...
            neg_feats, neg_classes = self.fetch_feats(neg_ann_ids, 1, opt)
    ...
    def getImageBatch(self, split, opt):
        ...
        ann_ids = image['ann_ids']
        feats, classes = self.fetch_feats(ann_ids, 1, opt)
        sent_ids = []
        gd_ixs = []
        rank = []
    ...
    def getTestBatch(self, split, opt):
        ...
        feats, classes = self.fetch_feats(batch_ann_ids, 1, opt)
        seqz = self.fetch_seqs(batch_sent_ids, {'pad_zero':'end'})
        zseq = self.fetch_seqs(batch_sent_ids, {'pad_zero':'front'})
        data = {}
